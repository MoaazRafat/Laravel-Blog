...
<div class="container m-auto text-center pt-15 pb-5">
    ...
    <div class="mt-2">
        By: <span class="text-gray-500 italic">{{$post->user->name}}</span> on:
        <span
            class="text-gray-500 italic">{{date('m-d-Y',strtotime($post->updated_at))}}</span>
    </div>
</div>
<div class="container m-auto pt-15 pb-5 leading-6">
    <div class="flex h-96">
        ...
        <form action="{{route('blog.destroy',$post->slug)}}" method="post">
            ...
            <button type="submit"
                class="bg-red-700 text-gray-100
                        py-4 px-5 rounded-lg
                        font-bold uppercase text-l
                        place-self-start">Delete Post</button>
        </form>
    </div>
    @endif
